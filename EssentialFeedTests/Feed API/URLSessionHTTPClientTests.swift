...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    func test_getFromURL_succeedsOnHTTPURLResponseWithData() {
        ...
        makeSUT().get(from: anyURL()) { result in
            ...
            default:
                XCTFail("Expected success with data \(data) and response \(response), got \(result) instead.")
            }
            exp.fulfill()
        }
    ...
    func test_getFromURL_succeedsWithEmptyDataOnHTTPURLResponseWithNilData() {
        ...
        makeSUT().get(from: anyURL()) { result in
            ...
            case .success(let receivedData, let receivedResponse):
                ...
                XCTAssertEqual(receivedResponse.statusCode, response.statusCode)
            default:
                XCTFail("Expected success with empty data and response \(response), got \(result) instead.")
            }
            exp.fulfill()
        }
    ...
    private func resultErrorFor(data: Data?, response: URLResponse?, error: Error?, file: StaticString = #filePath, line: UInt = #line) -> Error? {
        ...
        sut.get(from: anyURL()) { result in
            ...
            case .failure(let error):
                receivedError = error
            default:
                XCTFail("Expected failure with error \(String(describing: error)), got \(result) instead.", file: #filePath, line: #line)
            }
